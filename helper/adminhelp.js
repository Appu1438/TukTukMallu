...
module.exports={
    ...
    doSignup:(admindata)=>{
        ...
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.ADMIN_COLLECTION).updateOne({Email:data.Email},
                {
                    $set:{
                        Pass:data.Pass
                    }
      ...
      updatecount:(counts)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.COUNTS_COLLECTION).updateOne({},{
                $set:{
                     Dcount:counts.Dcount,
    ...
    countPendingRequests:()=>{
      ...
      getcount:()=>{
        return new Promise(async(resolve,reject)=>{
           let counts=await db.get().collection(collection.COUNTS_COLLECTION).find().toArray()
           resolve(counts)
        })
      },
      addcount:(counts)=>{
        ...
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.COUNTS_COLLECTION).insertOne(countobj).then(()=>{
                resolve()
            })
        })
