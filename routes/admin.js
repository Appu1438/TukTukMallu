...
router.get('/', verifylogin, async (req, res, next) => {
  ...
  let counts = await adminhelp.getcount()
  console.log(count)
  console.log(usercount)
  console.log(counts)
  res.render('admin/index', { admin: true, admins, count, usercount,counts });
});
...
router.post('/signup', async (req, res) => {
  ...
  if (existingUser) {
    ...
    res.render('admin/signup', { error: 'Email already exists', admin: true });
  } else {
    ...
    transporter.sendMail(mailOptions, async (error, info) => {
      if (error) {
        console.error(error);
        res.render('admin/signup', { error: 'Error sending OTP. Please try again.', admin: true });
      } else {
        ...
        res.render('admin/verify', { email: userData.Email, otp, userData, admin: true });
      }
    });
  }
});
...
router.post('/verify-otp', (req, res) => {
  ...
  if (otp == storedOTP) {
    ...
    adminhelp.doSignup(req.body).then((response) => {
      ...
      .catch((error) => {
        res.render('admin/signup', { error: 'Error signing up. Please try again.', admin: true });
      });
  } else {
    // Invalid OTP, show an error message
    res.render('admin/verify', { Email, error: 'Invalid OTP. Please try again.', admin: true });
  }
});
router.get('/forgot', (req, res) => {
  res.render('admin/forgot', { admin: true })
})
router.post('/forgot', (req, res) => {
  ...
  const mailOptions = {
    ...
    text: `Your OTP for Update TukTuk Admin Password is: ${otp}`,
  };

  transporter.sendMail(mailOptions, async (error, info) => {
    if (error) {
      console.error(error);
      res.render('admin/forgot', { error: 'Error sending OTP. Please try again.', admin: true });
    } else {
      ...
      res.render('admin/verifypass', { email: userData.Email, otp, userData, admin: true });
    }
  });
...
router.post('/verify-otp-pass', (req, res) => {
  ...
  if (otp == storedOTP) {
    ...
    delete req.session.otp;
    adminhelp.UpdatePass(req.body)
      .then((response) => {
        res.redirect('/admin');
      })
      .catch((error) => {
        res.render('admin/forgot', { error: 'Error Updating Password. Please try again.', admin: true });
      });
  } else {
    // Invalid OTP, show an error message
    res.render('admin/verifypass', { Email, error: 'Invalid OTP. Please try again.', admin: true });
  }
});
...
router.get('/addcount', async (req, res) => {
  ...
  let count = await adminhelp.countPendingRequests()
  let usercount = await adminhelp.countUsers()
  res.render('admin/addcount', { admin: true, admins, count, usercount })
})
router.post('/addcount', (req, res) => {
  adminhelp.addcount(req.body).then(() => {
    res.redirect('/admin')
  })
})
router.get('/changecount', async (req, res) => {
  let admins = req.session.admin
 
  let usercount = await adminhelp.countUsers()
  let counts = await adminhelp.getcount()
  console.log(counts)
  res.render('admin/changecount', { admin: true, admins, usercount, counts })
})
router.post('/changecount',(req,res)=>{
  adminhelp.updatecount(req.body).then(()=>{
    res.redirect('/admin')
  })
})
